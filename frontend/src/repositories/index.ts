...
export const authRepository = {
  async login(email: string, password_hash: string): Promise<{ access_token: string; refresh_token: string; user: User }> {
    const response = await apiClient.post('/api/v1/auth/login', { email, password_hash });
